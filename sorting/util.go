...
func swap[T comparable](a []T, i, j int) {
	contract.Require(0 <= i && i < len(a), "i is within bound")
	contract.Require(0 <= j && j < len(a), "j is within bound")
	defer func(oldAi, oldAj T) {
		contract.Ensure(a[i] == oldAj && a[j] == oldAi, "elements at i, j are swapped")
	}(a[i], a[j])
...
func rangeLessOrEqual[T comparable](a []T, lowA, highA int, b []T, lowB, highB int, comp order.CompareFn[T]) bool {
	contract.Require(0 <= lowA && lowA <= highA && highA <= len(a), "lowA and highA are within bound")
	contract.Require(0 <= lowB && lowB <= highB && highB <= len(b), "lowB and highB are within bound")
